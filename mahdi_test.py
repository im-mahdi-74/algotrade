...
def sell_up(symbol , lot , tp_ , sl_ , reward):
    

    tp = abs(mt5.symbol_info_tick(symbol).ask - tp_ -  (tp_ / reward ) ) 
    sl = abs(mt5.symbol_info_tick(symbol).bid + sl_ - ( tp_ / reward) ) 
    ...
    lot = round_up(lot, 2)
    point = mt5.symbol_info(symbol).point
    price = mt5.symbol_info_tick(symbol).ask - ( tp_ /reward)
    deviation = 100
    request = {
...
def sell_down(symbol , lot , tp_ , sl_ ):

    
    
    tp = abs(mt5.symbol_info_tick(symbol).ask - tp_  ) 
    sl = abs(mt5.symbol_info_tick(symbol).bid + sl_ + tp_  ) 
...
def buy_down(symbol , lot , tp_ , sl_  , reward):
...
def trade_up(symbol , lot , tp , sl , reward ):
    ...
    while True:
        ...
        if   positions_total == 0 and init_position  :
            ...
            log , doit = buy_up( symbol, lot ,  ( tp / reward) ,  tp  + ( tp / reward) )
        ...
        if  level_sell_count < level_buy_count and numtrade == 1  and init_sell  :
            ...
            while True:
                ...
                log , doit =     sell_up( symbol, level_vol_sell ,  tp ,  tp / reward  , reward)               
                if doit :
                    ...
                    print(f' Done position {log} ')
                    break

                else : 
                    print(f' falid position {log} ')
                    time.sleep(0.1)
                    continue
...
def trade_down(symbol , lot , tp , sl , reward):
    ...
    while True:
        ...
        if   positions_total == 0 and init_position  :
            ...
            log , doit = sell_down( symbol, lot ,  ( tp / reward) , tp +  ( tp / reward)  )
        ...
        if  level_sell_count < level_buy_count and numtrade == 1  and init_sell  :
            ...
            while True:
                ...
                log , doit =     buy_down( symbol, level_vol_sell ,  tp ,  tp / reward , reward )               
                if doit :
...
def run():
    ...
    while True:
        ...
        if 9 < now.hour < 22:
            try:
                gbp_per, gbp_mult = 25 , 1
                gbp_signal = scalping_strategy('GBPUSD_o' , gbp_per, gbp_mult)
                if gbp_signal == 'buy' and len(mt5.positions_get(symbol='GBPUSD_o')) == 0:
                    th.Thread(target=trade_up, args=('GBPUSD_o', 1 , 0.0003, 0.01 , 2)).start()
                if gbp_signal == 'sell' and len(mt5.positions_get(symbol='GBPUSD_o')) == 0:
                    th.Thread(target=trade_down, args=('GBPUSD_o',  1  , 0.0003, 0.01 , 2)).start()
                time.sleep(1)

                # jpy_signal = scalping_strategy('USDJPY_o')
                # if jpy_signal == 'buy' and len(mt5.positions_get(symbol='USDJPY_o')) == 0:
                #     th.Thread(target=trade_up, args=('USDJPY_o', 0.01, 0.05, 0.05)).start()
                # if jpy_signal == 'sell' and len(mt5.positions_get(symbol='USDJPY_o')) == 0:
                #     th.Thread(target=trade_down, args=('USDJPY_o', 0.01, 0.05, 0.05)).start()
                # time.sleep(1)

                # chf_signal = scalping_strategy('USDCHF_o')
                # if chf_signal == 'buy' and len(mt5.positions_get(symbol='USDCHF_o')) == 0:
                #     th.Thread(target=trade_up, args=('USDCHF_o', 0.01, 0.0005, 0.0006)).start()
                # if chf_signal == 'sell' and len(mt5.positions_get(symbol='USDCHF_o')) == 0:
                #     th.Thread(target=trade_down, args=('USDCHF_o', 0.01, 0.0005, 0.0006)).start()
                # time.sleep(1)
                ...
                # cad_signal = scalping_strategy('USDCAD_o')
                # if cad_signal == 'buy' and len(mt5.positions_get(symbol='USDCAD_o')) == 0:
                #     th.Thread(target=trade_up, args=('USDCAD_o', 0.01, 0.0005, 0.0006)).start()
                # if cad_signal == 'sell' and len(mt5.positions_get(symbol='USDCAD_o')) == 0:
                #     th.Thread(target=trade_down, args=('USDCAD_o', 0.01, 0.0005, 0.0006)).start()
                # time.sleep(1)
