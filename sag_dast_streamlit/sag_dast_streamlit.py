...
def buy(symbol , lot , tp_ , sl_  , price):
...
def sell(symbol , lot , tp_ , sl_  , price):
...
def sell_stop(symbol , lot , tp_ , sl_ , reward , price):
    

    tp = abs(  price  - tp_ - reward ) 
    sl = abs(  price   ) 
...
def buy_stop(symbol , lot , tp_ , sl_  , reward , price):
...
def trade_up(symbol , lot , tp , sl , reward , price  ):
    zareb = 0.5
    numtrade = 0
    # up_numtrade = 0
    sl = sl 
    ...
    while True:
        ...
        if   len(mt5.positions_get(symbol=symbol)) == 0 and init_position  :
            ...
            log , doit = buy( symbol, lot ,  tp ,  sl , price)
            sell_init  = True
            level_buy_count += 1
            position_count += 1
            level_vol_buy = lot
        ...
        if   len(mt5.positions_get(symbol=symbol)) == 1   :
            if mt5.positions_get(symbol=symbol)[0].type == 0 and sell_init : 
                ...
                buy_init = True
                lot = lot + (lot * zareb)
                log , doit = sell_stop( symbol, lot ,  tp ,  sl , sl , price )   
                while True : 
                    if not doit:
                        log , doit = sell_stop( symbol, lot ,  tp ,  sl , sl , price  ) 
                        continue
            ...
            if mt5.positions_get(symbol=symbol)[0].type == 1 and buy_init :
                ...
                sell_init = True
                lot = lot + (lot * zareb)
                log , doit = buy_stop( symbol, lot ,  tp ,  sl , sl  , price )
                while True : 
                    if not doit:
                        log , doit = buy_stop( symbol, lot ,  tp ,  sl , sl  , price ) 
                        continue
...
def trade_down(symbol , lot , tp , sl , reward , price ):
    zareb = 0.5
    numtrade = 0
    # up_numtrade = 0
    sl = sl 
    ...
    while True:
        ...
        if   len(mt5.positions_get(symbol=symbol)) == 0 and init_position  :
            ...
            log , doit = sell( symbol, lot ,  tp ,  sl , price )
            sell_init  = True
            level_buy_count += 1
            position_count += 1
            level_vol_buy = lot
...
def scalping_strategy(symbol,  per, mult):
    ...
    for i in range(1, len(df)):
        ...
        # sell_signal = (df['close'][i] < df['upper_band'][i]) and (df['rsi'][i] < 70) and (df['macd'][i] < df['signal'][i]) and (df['sma_short'][i] < df['sma_long'][i])
            ...
            if sell_signal:
                return 'sell' , mt5.symbol_info_tick(symbol).bid
            else:
    ...
    while True:
        ...
        if 9 < now.hour < 22 :
            try:
                ...
                gbp = scalping_strategy('GBPUSD_o' , gbp_per, gbp_mult)
                if gbp is not None:
                    gbp_signal , gbp_price = gbp
                
                    if gbp_signal == 'buy' and len(mt5.positions_get(symbol='GBPUSD_o')) == 0:
                        th.Thread(target=trade_up, args=('GBPUSD_o', 0.1, 0.0005, 0.0002 , 3 , gbp_price)).start()
                    if gbp_signal == 'sell' and len(mt5.positions_get(symbol='GBPUSD_o')) == 0:
                        th.Thread(target=trade_down, args=('GBPUSD_o', 0.1, 0.0005, 0.0002 , 3 , gbp_price)).start()
                time.sleep(1)

                gbp_per, gbp_mult = 50, 1
                jpy = scalping_strategy('USDJPY_o' , gbp_per, gbp_mult)
                if jpy is not None:
                    jpy_signal , jpy_price = scalping_strategy('USDJPY_o' ,  gbp_per, gbp_mult)
                    if jpy_signal == 'buy' and len(mt5.positions_get(symbol='USDJPY_o')) == 0:
                        th.Thread(target=trade_up, args=('USDJPY_o', 0.1, 0.05, 0.02 , 3 , jpy_price)).start()
                    if jpy_signal == 'sell' and len(mt5.positions_get(symbol='USDJPY_o')) == 0:
                        th.Thread(target=trade_down, args=('USDJPY_o', 0.1, 0.05, 0.02 , 3 , jpy_price)).start()
                time.sleep(1)

                gbp_per, gbp_mult = 50, 1
                chf = scalping_strategy('USDCHF_o' , gbp_per, gbp_mult)
                if chf is not None:
                    chf_signal , chf_price = chf
                    if chf_signal == 'buy' and len(mt5.positions_get(symbol='USDCHF_o')) == 0:
                        th.Thread(target=trade_up, args=('USDCHF_o', 0.1, 0.0005, 0.0002 , 3 , chf_price)).start()
                    if chf_signal == 'sell' and len(mt5.positions_get(symbol='USDCHF_o')) == 0:
                        th.Thread(target=trade_down, args=('USDCHF_o', 0.1, 0.0005, 0.0002 , 3 , chf_price)).start()
                time.sleep(1)
