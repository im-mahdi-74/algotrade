...
def trade(symbol , lot , tp , sl):
    ...
    while True:
        ...
        if   positions_total == 0 and init_position :
            ...
            level_sell = abs(mt5.symbol_info_tick(symbol).ask - (tp )) 
        ...
        if mt5.symbol_info_tick(symbol).ask   <= level_sell  and level_sell_count < level_buy_count and numtrade == 1  and init_sell  :
            ...
            position_count += 1
            level_vol_sell  = level_vol_buy * 3
...
def main():
    ...
    th.Thread(target= trade , args= ( 'USDJPY_o' ,  0.35 , 0.03   , 0.05 )).start()
    time.sleep(0.1)    
    th.Thread(target= trade , args= ( 'GBPUSD_o' ,  0.35 , 0.0003, 0.01 )).start()
    time.sleep(0.1)
    th.Thread(target= trade , args= ( 'AUDUSD_o' ,  0.35 , 0.0003 , 0.01 )).start()
    time.sleep(0.1)
    th.Thread(target= trade , args= ( 'NZDUSD_o' ,  0.35 , 0.0003 , 0.01 )).start()
    time.sleep(0.1)
    th.Thread(target= trade , args= ( 'EURUSD_o' ,  0.35 , 0.0003 , 0.01 )).start()
...
def run():
    ...
    while True:
        time.sleep(1)
        print('run' , datetime.datetime.now(tehran_timezone))
        ...
        # if now.hour == 5 or now.hour == 7 or now.hour == 11 :
        #     if now.minute == 00 :
        #         main()
        #         time.sleep(100)           
        ...
        if now.minute == 30 or now.minute == 45 or now.minute == 15 or now.minute == 1 :
            main()
            time.sleep(100)
