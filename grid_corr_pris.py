...
def close_pos(tickit_one_one, tickit_one_tow, tickit_tow_one, tickit_tow_tow ):
    ...
    while True:
        time.sleep(0.05)
        sod = mt5.positions_get(ticket=tickit_one_one.order)[0].volume * 20
    ...
    if 0 < now.hour < 24   :
        try:
            ...
            if len(mt5.positions_get(symbol='GBPUSD_o')) == 0 and len(mt5.positions_get(symbol='EURUSD_o')) == 0 :
                tickit_one_one =  buy('GBPUSD_o', 0.19 , 0 , 0 , 'one')
                tickit_one_tow =  sell('EURUSD_o', 0.22 , 0 , 0 , 'one')
                tickit_tow_one =  sell('GBPUSD_o', 0.19 , 0 , 0 , 'tow')
                tickit_tow_tow =  buy('EURUSD_o', 0.22 , 0 , 0 , 'tow')
