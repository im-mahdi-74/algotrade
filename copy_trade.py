...
acc_slave , server_slave , password_slave =   2145904742  , 'EightcapLtd-Demo3' , 'H4TzE2Xh'
adres_slave =  r"C:\Program Files (x86)\Eightcap MetaTrader 4\terminal.exe"


# acc_slave , server_slave , password_slave =   52029351  , 'Alpari-MT5-Demo' , 'Mahdi1400@'
# adres_slave =  r"C:\Program Files\Alpari MT5\terminal64.exe"



acc_master , server_master , password_master =    89045253 , 'LiteFinance-MT5-Demo' , 'Mahdi1400@'
adres_master = r"C:\Program Files\LiteFinance MT5 Terminal\terminal64.exe"


init_master = mt5.initialize( path = adres_master)
print('try')
login_master= mt5.login( login = acc_master , server =  server_master  , password = password_master  )
print(init_master)
print(login_master)
...
def to_ag( acc , server , password , adres , type , eu):

        
    if type == 'master':
        copy(acc , server , password , adres , type , eu).master()
    if type == 'slave':
        copy(acc , server , password , adres , type , eu).slave()
            
            
            

# th.Thread(target= to_ag , args=(acc_slave2 , server_slave2 , password_slave2 , adres_slave2 , 'master' , '')).start()
# th.Thread(target= to_ag , args=(acc_slave , server_slave , password_slave , adres_slave , 'slave' , '')).start()
# th.Thread(target= to_ag , args=(acc_master , server_master, password_master , adres_master , 'master' , '_o')).start()
