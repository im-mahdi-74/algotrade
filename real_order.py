...
def main():
    ...
    if len(mt5.positions_get( symbol = 'USDJPY_o' )) == 0:
        th.Thread(target= trade , args= ( 'USDJPY_o' ,  0.15 , 0.05   , 0.05 )).start()
    time.sleep(0.3)    

    if len(mt5.positions_get( symbol = 'GBPUSD_o' )) == 0:
        th.Thread(target= trade , args= ( 'GBPUSD_o' ,  0.1 , 0.0005, 0.01 )).start()
    time.sleep(0.3)
    ...
    if len(mt5.positions_get( symbol = 'EURUSD_o' )) == 0:
        th.Thread(target= trade , args= ( 'EURUSD_o' ,  0.1 , 0.0005 , 0.01 )).start()
    time.sleep(0.3)

    if len(mt5.positions_get( symbol = 'USDCAD_o' )) == 0:
        th.Thread(target= trade , args= ( 'USDCAD_o' ,  0.1 , 0.0005 , 0.01)).start()
    time.sleep(0.3)

    if len(mt5.positions_get( symbol = 'USDCHF_o' )) == 0:
        th.Thread(target= trade , args= ( 'USDCHF_o' ,  0.1 , 0.0005 , 0.0006 )).start()
...
def run():
    ...
    while True:
        ...
        #     if now.minute == 30 or now.minute == 45 or now.minute == 0 :
        main()
        time.sleep(300)
